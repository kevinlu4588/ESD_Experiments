...
class StableDiffuser(torch.nn.Module):
    def __init__(self, scheduler='DDIM'):
        super().__init__()
    ...
    def get_noise(self, batch_size, img_size, generator=None):
        ...
        return torch.randn((batch_size, self.unet.in_channels, img_size // 8, img_size // 8), generator=generator).type(param.dtype).to(param.device)
    ...
    def predict_noise(self, iteration, latents, text_embeddings, guidance_scale=7.5):
        ...
        noise_prediction = self.unet(latents, self.scheduler.timesteps[iteration], encoder_hidden_states=text_embeddings).sample
    ...
    @torch.no_grad()
    def diffusion(self, latents, text_embeddings, end_iteration=1000, start_iteration=0, return_steps=False, pred_x0=False, trace_args=None, show_progress=True, **kwargs):
        latents_steps = []
        trace_steps = []
        trace = None
        for iteration in tqdm(range(start_iteration, end_iteration), disable=not show_progress):
            ...
            noise_pred = self.predict_noise(iteration, latents, text_embeddings, **kwargs)
    ...
    @torch.no_grad()
    def continue_diffusion(self, latents, new_prompts, start_iteration, end_iteration, n_imgs=1, **kwargs):
        new_text_embeddings = self.get_text_embeddings(new_prompts, n_imgs=n_imgs)
        latents_steps, trace_steps = self.diffusion(latents, new_text_embeddings, end_iteration=end_iteration, start_iteration=start_iteration, **kwargs)
        return latents_steps, trace_steps

    @torch.no_grad()
    def __call__(self, prompts, second_prompts, marker, img_size=512, n_steps=50, n_imgs=1, end_iteration=None, generator=None, **kwargs):
        ...
        latents_steps, trace_steps = self.diffusion(latents, text_embeddings, end_iteration=marker, **kwargs)

        #Second half
        final_latents_steps, _ = self.continue_diffusion(latents_steps[-1], [second_prompts], start_iteration=marker, end_iteration=n_steps, **kwargs)
        final_latents_steps = [self.decode(latents.to(self.unet.device)) for latents in latents_steps]
        images_steps = [self.to_image(latents) for latents in final_latents_steps]
        ...
        for i in range(len(images_steps)):
            ...
            image, has_nsfw_concept = self.safety_checker(images=latents_steps[i].float().cpu().numpy(), clip_input=safety_checker_input.pixel_values.float())
...
if __name__ == '__main__':
    diffuser = StableDiffuser(scheduler='DDIM').to('gpu')

    prompt1 = "a cat"
    prompt2 = "a dog"
    n_steps = 50
    halfway_step = n_steps // 2

    # Initial diffusion with the first prompt
    diffuser.set_scheduler_timesteps(n_steps)
    latents = diffuser.get_initial_latents(1, 512, 1)
    text_embeddings = diffuser.get_text_embeddings([prompt1], n_imgs=1)
    latents_steps, _ = diffuser.diffusion(latents, text_embeddings, end_iteration=halfway_step, guidance_scale=7.5)

    # Continue diffusion with the second prompt
    final_latents_steps, _ = diffuser.continue_diffusion(latents_steps[-1], [prompt2], start_iteration=halfway_step, end_iteration=n_steps, guidance_scale=7.5)

    final_images = [diffuser.to_image(latents.to(diffuser.unet.device)) for latents in final_latents_steps]
    final_images = list(zip(*final_images))
